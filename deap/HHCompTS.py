...
def random_household(index):
    pop = getdictionary(popdf)
    ...
    for s in range(size):
        # household['persons'] += get_random_person(random.choice(['E', 'A', 'C']), 1)
        persons_list = []
        if len(pop) > 0:
            persons = random.choices(pop, k=1)
            for person in persons:
                [pop.remove(x) for x in pop if x['id'] == person['id']]
                persons_list.append(person['id'])
            household['persons'] += persons_list
...
def random_solution():
    ...
    for i in range(HH_Total):
        ...
        # if composition == '1E_0C':
        #     household['persons'] = get_random_person('E', 1)
        # elif composition == '1A_0C':
        #     household['persons'] = get_random_person('A', 1)
        # elif composition == '1A_1C':
        #     persons = get_random_person('A', 1)
        #     persons += get_random_person('C', 1)
        #     household['persons'] = persons
        # elif composition == '2E_0C':
        #     persons = get_random_person('E', 2)
        #     household['persons'] = persons
        # elif composition == '2A_0C':
        #     persons = get_random_person('A', 2)
        #     household['persons'] = persons
        # elif composition == '2A_1C':
        #     persons = get_random_person('A', 2)
        #     persons += get_random_person('C', random.randint(1, size - 2))
        #     household['persons'] = persons
        # elif composition == '2A_3C':
        #     persons = get_random_person('A', 2)
        #     persons += get_random_person('C', random.randint(3, size - 2))
        #     household['persons'] = persons
        # elif composition == '3A_1C':
        #     persons = get_random_person('A', 3)
        #     persons += get_random_person('C', random.randint(1, size - 3))
        #     household['persons'] = persons
        # elif composition == '3A_0C':
        #     persons = get_random_person('A', random.randint(3, size))
        #     household['persons'] = persons
...
def tabu_search(initial_solution, objective_function, tabu_size, max_iterations):
    ...
    for i in range(max_iterations):
        neighbors = generate_neighbor(current_solution)  # Generate neighboring solutions
        best_neighbor = None
...
toolbox.register("tabu_search", tabu_search, initial_solution, evaluate,
                 tabu_size=100, max_iterations=200)
