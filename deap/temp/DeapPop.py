...
def getStats(persons):
    ...
    for person in persons:
        temp.append(person.age + " " + person.sex)
    predicted_sex_by_age = Counter(temp)
    return dict([(k, predicted_sex_by_age[k]) for k in list(actual.keys())])

# Create fitness function (the closer to the target, the better)
def fitness(individual):
    predicted = getStats(individual)
...
def random_person():
    ...
    person = Person(id=0, age=age, sex=sex, ethnicity='A1', religion='M', status='Single',
