...
pareto_array = np.array([ind.fitness.values for ind in hof])
n_objectives = pareto_array.shape[1]
num_objectives = len(pareto_array[0])
weights = np.ones(num_objectives) / num_objectives
weighted_sums = np.dot(pareto_array, weights)
best_index = np.argmin(weighted_sums)
...
best_solution = hof[best_index]
# print(best_solution)
# plot(CROSS_TABLES['sex_age'], getStats(best_solution, 'sex_age') ,'sex_age')
# plot(CROSS_TABLES['sex_age_ethnicity'], getStats(best_solution, 'sex_age_ethnicity') ,'sex_age_ethnicity', SHIFTLEFT=True)
# plot(CROSS_TABLES['sex_age_religion'], getStats(best_solution, 'sex_age_religion'), 'sex_age_religion')
# plot(CROSS_TABLES['sex_age_status'], getStats(best_solution, 'sex_age_status'), 'sex_age_status')
# plot(CROSS_TABLES['sex_age_qualification'], getStats(best_solution, 'sex_age_qualification'), 'sex_age_qualification')

best_solution = pareto_array[best_index]
fig, axes = plt.subplots(n_objectives, n_objectives, figsize=(15, 15))

for row in range(n_objectives):
    for col in range(n_objectives):
        if row == col:
            axes[row, col].text(0.5, 0.5, f'Objective {row + 1}', ha='center', va='center', fontsize=12)
            axes[row, col].set_xticks([])
            axes[row, col].set_yticks([])
        elif row > col:
            axes[row, col].scatter(pareto_array[:, col], pareto_array[:, row], s=5)
            axes[row, col].scatter(best_solution[0], best_solution[1], c="red", s=10)
            axes[row, col].set_xlabel(f'Objective {col + 1}')
            axes[row, col].set_ylabel(f'Objective {row + 1}')
        else:
