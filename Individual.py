...
class Individual:
    ...
    def generateAll(self):
        rsamples = ICT.get_weighted_samples(ICT.religion_by_sex_by_age, self.genelength)
        esamples = ID.get_weighted_samples(ID.ethnicdf, self.genelength)
        ssamples = ICT.get_weighted_samples(ICT.marital_by_sex_by_age, self.genelength)
        qsamples = ICT.get_weighted_samples(ICT.qualification_by_sex_by_age, self.genelength)

        persons = []
        for i in range(0, self.genelength):
            age = rsamples[i].split(' ')[1]
            sex = rsamples[i].split(' ')[0]
            ethnicity = esamples[i]
            religion = rsamples[i].split(' ')[2]
            status = ssamples[i].split(' ')[2]
            if age in ['0-4', '5-7', '8-9', '10-14', '15']:
                qual = 'no'
            else:
                qual = qsamples[i].split(' ')[2]
            person = Person(i + 1, age=age, sex=sex, ethnicity=ethnicity, religion=religion, status=status, qualification=qual)
    ...
    def cosine(self):
        actual = np.array(list(self.actual.values()))
        predicted = np.array(list(self.predicted.values()))
        return 1 - distance.cosine(actual, predicted)
    ...
    def randomize(self, gene_index):
        ...
        self.genes[gene_index].age, self.genes[rnd].age = self.genes[rnd].age, self.genes[gene_index].age
        self.genes[gene_index].sex, self.genes[rnd].sex = self.genes[rnd].sex, self.genes[gene_index].sex

    def violenplot(self):
        fig = go.Figure()
        fig.add_trace(
            go.Violin(x0='data', name='actual', y=list(self.actual.values()), line_color='#636EFA', side='negative'))
        fig.add_trace(
            go.Violin(x0='data', name='pred', y=list(self.predicted.values()), line_color='#EF553B', side='positive'))
